...
{
    const int class_count = 1;//CLASSES
    Mat data;
    Mat responses;

    bool ok = read_num_class_data( data_filename, 24, &data, &responses );//third parameter: FEATURES
    if( !ok )
        return ok;
    ...
    else
    {
        ...
        // 2. train classifier
        int layer_sz[] = { data.cols, 75,50,40, class_count };
